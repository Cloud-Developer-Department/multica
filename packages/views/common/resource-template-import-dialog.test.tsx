...
import type { ResourceTemplateDoc } from "@multica/core/types";
...
describe("ResourceTemplateImportDialog", () => {
  ...
  it("fires onSuccess only after a successful apply, not on close/cancel", async () => {
    ...
    applySpy.mockResolvedValue({
      applied: true,
      dry_run: false,
      created: { agents: [{ ref: "My Agent", id: "ag-1", name: "My Agent" }], squads: [], skills: [] },
      resource_mapping: {},
      rolled_back: false,
      idempotent_replay: false,
    });
    ...
    renderWithI18n(
      <ResourceTemplateImportDialog
        ...
        onSuccess={onSuccess}
        initialDocs={[AGENT_DOC as unknown as ResourceTemplateDoc]}
      />,
    );
    // initialDocs seeds the review step directly (marketplace import entry).
    await screen.findByText(/target runtime/i);
    fireEvent.change(screen.getByLabelText(/target runtime/i), {
      target: { value: "rt-1" },
    });
    ...
    // Cancelling (closing without applying) must NOT call onSuccess again.
    onSuccess.mockClear();
