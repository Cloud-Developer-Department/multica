...
/**
 * Grouped-by-issue list for the Issue Documents tab (CLO-471). Each issue is a
 * section whose header carries the issue identifier + title and whose rows are
 * the issue's flow documents (ordered server-side by the requested sort).
 */
            ...
            <TableBody>
              {group.items.map((doc) => (
                <TableRow
                  ...
                  // Keyboard accessibility (CLO-283 R3, §6.4): a clickable row
                  // must be focusable and activatable with Enter / Space.
                  tabIndex={0}
                  role="button"
                  onKeyDown={(e) => {
