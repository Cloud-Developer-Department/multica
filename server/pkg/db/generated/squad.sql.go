...
const clearChildSquadParents = `-- name: ClearChildSquadParents :execrows
UPDATE squad SET parent_squad_id = NULL, updated_at = now()
WHERE parent_squad_id = $1
`

func (q *Queries) ClearChildSquadParents(ctx context.Context, parentSquadID pgtype.UUID) (int64, error) {
	result, err := q.db.Exec(ctx, clearChildSquadParents, parentSquadID)
	if err != nil {
		return 0, err
	}
...
const clearSquadParent = `-- name: ClearSquadParent :execrows
UPDATE squad SET parent_squad_id = NULL, updated_at = now()
WHERE id = $1 AND parent_squad_id IS NOT NULL
`

func (q *Queries) ClearSquadParent(ctx context.Context, id pgtype.UUID) (int64, error) {
	result, err := q.db.Exec(ctx, clearSquadParent, id)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected(), nil
...
SELECT id, workspace_id, name, description, leader_id, creator_id, created_at, updated_at, archived_at, archived_by, avatar_url, instructions, parent_squad_id, upgrade_on_member_mention FROM squad WHERE id = $1
`
...
SELECT s.id, s.workspace_id, s.name, s.description, s.leader_id, s.creator_id, s.created_at, s.updated_at, s.archived_at, s.archived_by, s.avatar_url, s.instructions, s.parent_squad_id FROM squad s WHERE s.id = $1 AND s.workspace_id = $2
`
...
SELECT id, workspace_id, name, description, leader_id, creator_id, created_at, updated_at, archived_at, archived_by, avatar_url, instructions, parent_squad_id, upgrade_on_member_mention FROM squad WHERE workspace_id = $1 ORDER BY created_at ASC
`
...
const listChildSquadMembers = `-- name: ListChildSquadMembers :many
SELECT sm.id               AS id,
...
type ListChildSquadMembersRow struct {
	ID             pgtype.UUID        `json:"id"`
	SquadID        pgtype.UUID        `json:"squad_id"`
	MemberType     string             `json:"member_type"`
	MemberID       pgtype.UUID        `json:"member_id"`
	...
	ChildSquadName string             `json:"child_squad_name"`
}
...
func (q *Queries) ListChildSquadMembers(ctx context.Context, parentSquadID pgtype.UUID) ([]ListChildSquadMembersRow, error) {
	rows, err := q.db.Query(ctx, listChildSquadMembers, parentSquadID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListChildSquadMembersRow{}
	for rows.Next() {
		var i ListChildSquadMembersRow
		if err := rows.Scan(
			&i.ID,
			&i.SquadID,
			&i.MemberType,
...
const listChildSquadSummaries = `-- name: ListChildSquadSummaries :many
...
type ListChildSquadSummariesRow struct {
	ID          pgtype.UUID `json:"id"`
	Name        string      `json:"name"`
...
func (q *Queries) ListChildSquadSummaries(ctx context.Context, parentSquadID pgtype.UUID) ([]ListChildSquadSummariesRow, error) {
	rows, err := q.db.Query(ctx, listChildSquadSummaries, parentSquadID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListChildSquadSummariesRow{}
	for rows.Next() {
		var i ListChildSquadSummariesRow
		if err := rows.Scan(
			&i.ID,
			&i.Name,
			&i.MemberCount,
...
const listChildSquadSummariesByWorkspace = `-- name: ListChildSquadSummariesByWorkspace :many
...
type ListChildSquadSummariesByWorkspaceRow struct {
...
func (q *Queries) ListChildSquadSummariesByWorkspace(ctx context.Context, workspaceID pgtype.UUID) ([]ListChildSquadSummariesByWorkspaceRow, error) {
	rows, err := q.db.Query(ctx, listChildSquadSummariesByWorkspace, workspaceID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListChildSquadSummariesByWorkspaceRow{}
	for rows.Next() {
		var i ListChildSquadSummariesByWorkspaceRow
		if err := rows.Scan(
			&i.ParentSquadID,
			&i.ID,
			&i.Name,
...
SELECT id, workspace_id, name, description, leader_id, creator_id, created_at, updated_at, archived_at, archived_by, avatar_url, instructions, parent_squad_id, upgrade_on_member_mention FROM squad WHERE workspace_id = $1 AND archived_at IS NULL ORDER BY created_at ASC
`
...
SELECT s.id, s.workspace_id, s.name, s.description, s.leader_id, s.creator_id, s.created_at, s.updated_at, s.archived_at, s.archived_by, s.avatar_url, s.instructions, s.parent_squad_id, s.upgrade_on_member_mention FROM squad s
...
const listSquadsByIdsInWorkspace = `-- name: ListSquadsByIdsInWorkspace :many
SELECT id, workspace_id, name, description, leader_id, creator_id, created_at, updated_at, archived_at, archived_by, avatar_url, instructions, parent_squad_id, upgrade_on_member_mention FROM squad WHERE id = ANY($1::uuid[]) AND workspace_id = $2
`

type ListSquadsByIdsInWorkspaceParams struct {
	ID          []pgtype.UUID `json:"id"`
	WorkspaceID pgtype.UUID   `json:"workspace_id"`
}
...
// one-level nesting).
func (q *Queries) ListSquadsByIdsInWorkspace(ctx context.Context, arg ListSquadsByIdsInWorkspaceParams) ([]Squad, error) {
	rows, err := q.db.Query(ctx, listSquadsByIdsInWorkspace, arg.ID, arg.WorkspaceID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const listSquadsLedByAgent = `-- name: ListSquadsLedByAgent :many
SELECT id, workspace_id, name, description, leader_id, creator_id, created_at, updated_at, archived_at, archived_by, avatar_url, instructions, parent_squad_id, upgrade_on_member_mention FROM squad WHERE leader_id = $1 AND workspace_id = $2 AND archived_at IS NULL ORDER BY created_at ASC
`

type ListSquadsLedByAgentParams struct {
	LeaderID    pgtype.UUID `json:"leader_id"`
	WorkspaceID pgtype.UUID `json:"workspace_id"`
}
...
func (q *Queries) ListSquadsLedByAgent(ctx context.Context, arg ListSquadsLedByAgentParams) ([]Squad, error) {
	rows, err := q.db.Query(ctx, listSquadsLedByAgent, arg.LeaderID, arg.WorkspaceID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const setSquadParent = `-- name: SetSquadParent :execrows
UPDATE squad SET parent_squad_id = $2, updated_at = now()
WHERE id = $1
`

type SetSquadParentParams struct {
	ID            pgtype.UUID `json:"id"`
	ParentSquadID pgtype.UUID `json:"parent_squad_id"`
}
...
func (q *Queries) SetSquadParent(ctx context.Context, arg SetSquadParentParams) (int64, error) {
	result, err := q.db.Exec(ctx, setSquadParent, arg.ID, arg.ParentSquadID)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected(), nil
...
UPDATE squad SET
    ...
    upgrade_on_member_mention = COALESCE($7, upgrade_on_member_mention),
    updated_at = now()
WHERE id = $1
RETURNING id, workspace_id, name, description, leader_id, creator_id, created_at, updated_at, archived_at, archived_by, avatar_url, instructions, parent_squad_id, upgrade_on_member_mention
`
