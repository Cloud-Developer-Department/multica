...
INSERT INTO issue (
    ...
    $16, $17
) RETURNING id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, origin_type, origin_id, first_executed_at, start_date, metadata, stage, properties, delegation_comment_id
`
...
const hasDelegatedChildIssues = `-- name: HasDelegatedChildIssues :one
SELECT EXISTS (
    SELECT 1 FROM issue
...
type HasDelegatedChildIssuesParams struct {
	DelegationCommentID pgtype.UUID `json:"delegation_comment_id"`
	WorkspaceID         pgtype.UUID `json:"workspace_id"`
}

func (q *Queries) HasDelegatedChildIssues(ctx context.Context, arg HasDelegatedChildIssuesParams) (bool, error) {
	row := q.db.QueryRow(ctx, hasDelegatedChildIssues, arg.DelegationCommentID, arg.WorkspaceID)
	var has_children bool
	err := row.Scan(&has_children)
	return has_children, err
}

const findActiveDelegatedChildIssue = `-- name: FindActiveDelegatedChildIssue :one
SELECT id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, origin_type, origin_id, first_executed_at, start_date, metadata, stage, properties, delegation_comment_id FROM issue
WHERE delegation_comment_id = $1
  AND assignee_id = $2
  AND workspace_id = $3
...
type FindActiveDelegatedChildIssueParams struct {
	DelegationCommentID pgtype.UUID `json:"delegation_comment_id"`
	AssigneeID          pgtype.UUID `json:"assignee_id"`
	WorkspaceID         pgtype.UUID `json:"workspace_id"`
}

func (q *Queries) FindActiveDelegatedChildIssue(ctx context.Context, arg FindActiveDelegatedChildIssueParams) (Issue, error) {
	row := q.db.QueryRow(ctx, findActiveDelegatedChildIssue,
		arg.DelegationCommentID,
		arg.AssigneeID,
		arg.WorkspaceID,
	)
