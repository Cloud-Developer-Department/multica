...
- **Issue 流程文档** — 位于「用量」下方的顶层标签页，按 issue 整合其研发流程各阶段产出的中间文档（需求、架构、测试报告、代码审查与安全审计结论、部署说明等），支持过滤、搜索与版本历史查看——开发者可集中追溯每条流程产出了哪些文档。文档由流程智能体或通过 CLI（`multica issue documents submit`）登记。
