...
看板（Board/Kanban）按状态分列，卡片可拖拽。父子 issue 以**树形**展示：

- **展开/折叠**：有子任务的父卡左上角有箭头，点击展开（子卡缩进显示在父卡下方）或折叠（整棵子树收起）。折叠状态会保存，且与表格视图的折叠状态互不影响。
- **拖拽语义**：
  - 拖父卡 = 整棵子树一起移动（同列排序 / 跨列改状态与负责人）；
  - 拖叶子子任务 = 单独移动；
  - 拖子任务跨列 = 脱离父任务，成为目标列顶层任务（拖拽预览会提示「将转为顶层任务」）。
- **子任务跟随父列**：子任务显示在父卡所在列，不受自身状态影响；仅当父卡不可见时才按自身状态落列。

> 关闭看板右上角「显示子 issue」开关后，仅显示根任务。
